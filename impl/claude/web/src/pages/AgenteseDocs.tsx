...
function AgenteseDocsInner() {
  ...
  // Ref to track if we should auto-invoke after path selection
  const shouldAutoInvoke = useRef(false);
  ...
  const handleSelectPath = useCallback((path: string) => {
    ...
    shouldAutoInvoke.current = true; // Flag for auto-invoke
  }, []);
  ...
  // Auto-invoke manifest when path is selected
  useEffect(() => {
    if (shouldAutoInvoke.current && selectedPath) {
      shouldAutoInvoke.current = false;
      handleInvoke('manifest');
    }
  }, [selectedPath, handleInvoke]);
