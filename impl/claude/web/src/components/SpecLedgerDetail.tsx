...
import { DirectorStatusBadge, AnalysisSummary, PromptDialog, CaptureDialog } from './director';
...
export function SpecLedgerDetail({
  ...
  // Director hook for lifecycle management
  const director = useDirector({ path, autoFetch: true });
  ...
  return (
    <div className="spec-ledger-detail">
      ...
      <div className="spec-ledger-detail__content">
        ...
        {/* Director Section */}
        <section className="spec-ledger-detail__section spec-ledger-detail__director">
          <div className="spec-ledger-detail__director-header">
            <h2 className="spec-ledger-detail__section-title">DIRECTOR</h2>
            {director.status && <DirectorStatusBadge status={director.status} showLabel />}
            {director.analyzedAt && (
              <span className="spec-ledger-detail__director-time">
                Analyzed {formatRelativeTime(director.analyzedAt)}
              </span>
            )}
          </div>

          <div className="spec-ledger-detail__director-actions">
            <button
              className="spec-ledger-detail__director-btn"
              onClick={() => director.analyze(path, true)}
              disabled={director.loading}
            >
              {director.loading ? 'Analyzing...' : 'Re-analyze'}
            </button>
            <button
              className="spec-ledger-detail__director-btn"
              onClick={() => setPromptDialogOpen(true)}
              disabled={!director.analysis}
            >
              Generate Prompt →
            </button>
            <button
          ...
          {director.analysis && <AnalysisSummary analysis={director.analysis} />}

          {director.error && (
            <p className="spec-ledger-detail__director-error">
      ...
      {/* Director Dialogs */}
      {promptDialogOpen && (
        <PromptDialog path={path} onClose={() => setPromptDialogOpen(false)} />
      )}
      {captureDialogOpen && (
        <CaptureDialog
          specPath={path}
          onClose={() => setCaptureDialogOpen(false)}
          onSuccess={() => director.refresh()}
        />
      )}
    </div>
  );
