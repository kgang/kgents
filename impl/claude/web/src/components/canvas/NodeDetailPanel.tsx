...
const CONTEXT_EXPLANATIONS: Record<string, { title: string; description: string; color: string }> =
  {
    world: {
      title: 'The External',
      description:
        'Entities, environments, and tools that exist outside the agent. This is where the agent interacts with reality.',
      color: 'blue',
    },
    self: {
      title: 'The Internal',
      description:
        'Memory, capability, and state within the agent. This is where introspection and self-awareness live.',
      color: 'violet',
    },
    concept: {
      title: 'The Abstract',
      description:
        'Platonic forms, definitions, and logical structures. Ideas that exist independently of instantiation.',
      color: 'emerald',
    },
    void: {
      title: 'The Accursed Share',
      description:
        'Entropy, serendipity, and gratitude. The part that must be spent without return. Embrace randomness.',
      color: 'gray',
    },
    time: {
      title: 'The Temporal',
      description:
        'Traces, forecasts, and schedules. How the past informs the present and shapes the future.',
      color: 'amber',
    },
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm font-medium text-gray-200 group-hover:text-white">{aspect}</div>
        {teachingMode && description && (
          <div className="text-[10px] text-gray-500 mt-0.5 line-clamp-2">{description}</div>
        )}
      </div>
...
function TeachingCallout({ context, children }: TeachingCalloutProps) {
  ...
  return (
    <div className={`rounded-lg border p-3 ${colorClasses[info.color] || colorClasses.gray}`}>
        ...
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {node.parent && (
                <div>
                  <h3 className="text-xs font-medium text-gray-500 uppercase mb-2">Hierarchy</h3>
            ...
            <div className="flex-shrink-0 p-4 border-t border-gray-800 bg-gray-900/50">
              <p className="text-[10px] text-gray-600 text-center">
                Press <kbd className="px-1 py-0.5 bg-gray-800 rounded text-gray-400">Esc</kbd> to
                close
              </p>
            </div>
          </motion.aside>
