...
/**
 * Get positive framing for compression/dropped items (QA-2)
 *
 * Philosophy: Compression is composting, not discarding.
 * Dropped items nourish the crystal, they're not waste.
 */
function getCompressionMessage(droppedCount: number): string {
  if (droppedCount === 0) {
...
function DisclosureBlock({ honesty, sizes }: DisclosureBlockProps) {
  ...
  if (honesty.dropped_count === 0) {
    return (
      ...
      >
        Everything was preserved in this crystal.
      </div>
    );
  }

  const compressionMessage = getCompressionMessage(honesty.dropped_count);
    ...
    <div>
      ...
      {showDetails && (
        <div
          ...
          style={{
            transition: `height ${TIMING.standard}ms, opacity ${TIMING.standard}ms`,
          }}
        >
          <div
          ...
          >
            {/* Composted themes (positive framing for dropped tags) */}
            ...
            {/* Composted moments preview (positive framing) */}
...
export function CrystalCard({
  ...
  // Expansion state reserved for future use
  void _controlledExpanded;
