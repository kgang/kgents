...
Migrated from agents/town/budget_store.py to services/ per Metaphysical
...
AGENTESE aspects:
- self.budget.manifest - Show user budget status
- self.budget.spend    - Spend credits
- world.town.consent   - Get consent state for citizen
...
# Subscription tiers per unified-v2.md
SUBSCRIPTION_TIERS = {
...
class InMemoryBudgetStore:
    ...
    async def get_or_create(self, user_id: str, tier: str = "TOURIST") -> UserBudgetInfo:
        ...
        budget = await self.get_budget(user_id)
        if budget is None:
            budget = await self.create_budget(user_id, tier)
        return budget
...
class RedisBudgetStore:
    ...
    async def _get_redis(self) -> Any:
        ...
        Returns Redis client if available, raises ImportError if not installed.
        ...
        if self._redis is None:
            try:
                ...
                logger.warning("redis package not installed - RedisBudgetStore unavailable")
                raise
