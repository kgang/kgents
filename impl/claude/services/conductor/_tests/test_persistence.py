...
class TestComposerPersistenceIntegration:
    """Integration tests with ChatMorpheusComposer."""

    @pytest.fixture
    def mock_morpheus(self) -> MagicMock:
    ...
    @pytest.mark.asyncio
    async def test_composer_loads_from_persistence(
        self, mock_morpheus: MagicMock, persistence: WindowPersistence
    ) -> None:
        """Composer loads window from D-gent when available."""
        from services.chat.composer import ChatMorpheusComposer

        # Pre-save a window
        pre_saved = ConversationWindow(max_turns=10, strategy="sliding")
        pre_saved.add_turn("Previous", "Context")
        await persistence.save_window("persistent-session", pre_saved)

        # Create composer with persistence
        composer = ChatMorpheusComposer(morpheus=mock_morpheus)
        composer.set_window_persistence(persistence)

        # Get window (should load from D-gent)
        session = self._make_session("persistent-session")
        window = await composer.get_or_create_window_async(session)

        # Verify it loaded the pre-saved state
        assert window.turn_count == 1
        turns = window.get_recent_turns()
        assert turns[0] == ("Previous", "Context")

    @pytest.mark.asyncio
    async def test_composer_saves_on_update(
        self, mock_morpheus: MagicMock, persistence: WindowPersistence
    ) -> None:
        """Composer saves window to D-gent on update_window_async."""
        from services.chat.composer import ChatMorpheusComposer

        composer = ChatMorpheusComposer(morpheus=mock_morpheus)
        composer.set_window_persistence(persistence)

        session = self._make_session("save-test")

        # Update should save
        await composer.update_window_async(session, "New message", "New response")

        # Verify it was saved
        assert await persistence.exists("save-test")
        loaded = await persistence.load_window("save-test")
        assert loaded is not None
