/**
 * TestAgent AGENTESE Hooks
 *
 * React hooks for world.test AGENTESE paths.
...
export interface TestManifestResponse {
  name: string;
  state: string;
  description: string;
}
...
/**
 * Fetch TestAgent manifest.
 * AGENTESE: world.test.manifest
 */
export function useTestQuery(): QueryResult<TestManifestResponse> {
  const { state, execute } = useAsyncState<TestManifestResponse>();

  const refetch = useCallback(() => {
    execute(fetchAgentese<TestManifestResponse>('world.test.manifest'));
  }, [execute]);
    ...
    async (data: InvokeRequest) => {
      ...
      try {
        const result = await execute(
          fetchAgentese<InvokeResponse>('world.test.invoke', data)
        );
