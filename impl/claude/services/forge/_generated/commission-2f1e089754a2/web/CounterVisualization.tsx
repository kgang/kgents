/**
 * CounterVisualization - Agent for: Build a simple counter that can increment and decrement
 *
 * Main visualization component for CounterAgent.
...
import { useCounterQuery } from './useCounterQuery';
...
export interface CounterVisualizationProps {
  /** Initial status data from PathProjection */
  status: CounterStatusData | null;
...
export interface CounterStatusData {
  name?: string;
  state?: string;
  description?: string;
}
...
export function CounterVisualization({
...
}: CounterVisualizationProps) {
  // AGENTESE hook for live data
  const manifest = useCounterQuery();

  // Derive status from manifest or initial data
  const status = manifest.data?.state || initialStatus?.state || 'unknown';
  const name = manifest.data?.name || initialStatus?.name || 'CounterAgent';
  const description = manifest.data?.description || initialStatus?.description || 'Agent for: Build a simple counter that can increment and decrement';
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <footer className="border-t border-stone-100 bg-white mt-16">
        <div className={`${maxWidth} mx-auto ${containerPadding} text-center`}>
          <p className="text-xs text-stone-300">CounterAgent</p>
        </div>
      </footer>
    </div>
  );
