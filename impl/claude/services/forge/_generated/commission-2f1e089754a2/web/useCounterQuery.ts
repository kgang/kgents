/**
 * CounterAgent AGENTESE Hooks
 *
 * React hooks for world.counter AGENTESE paths.
...
export interface CounterManifestResponse {
  name: string;
  state: string;
  description: string;
}
...
/**
 * Fetch CounterAgent manifest.
 * AGENTESE: world.counter.manifest
 */
export function useCounterQuery(): QueryResult<CounterManifestResponse> {
  const { state, execute } = useAsyncState<CounterManifestResponse>();

  const refetch = useCallback(() => {
    execute(fetchAgentese<CounterManifestResponse>('world.counter.manifest'));
  }, [execute]);
    ...
    async (data: InvokeRequest) => {
      ...
      try {
        const result = await execute(
          fetchAgentese<InvokeResponse>('world.counter.invoke', data)
        );
