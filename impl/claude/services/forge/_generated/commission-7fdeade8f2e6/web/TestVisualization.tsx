/**
 * TestVisualization - Agent for: Test agent
 *
 * Main visualization component for TestAgent.
...
export interface TestVisualizationProps {
  /** Initial status data from PathProjection */
  status: TestStatusData | null;
...
}: TestVisualizationProps) {
  // AGENTESE hook for live data
  const manifest = useTestQuery();

  // Derive status from manifest or initial data
  const status = manifest.data?.state || initialStatus?.state || 'unknown';
  const name = manifest.data?.name || initialStatus?.name || 'TestAgent';
  const description = manifest.data?.description || initialStatus?.description || 'Agent for: Test agent';
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <footer className="border-t border-stone-100 bg-white mt-16">
        <div className={`${maxWidth} mx-auto ${containerPadding} text-center`}>
          <p className="text-xs text-stone-300">TestAgent</p>
        </div>
      </footer>
    </div>
  );
