...
class PromptOutputFormatter:
    ...
    def format_compiled(
        ...
        for section in prompt.sections:
            ...
            lines.append(
                f"  {required_marker} {section.name}: {section.token_cost} tokens"
            )
        lines.append("")
    ...
    def format_history_timeline(
        ...
        for i, summary in enumerate(history):
            ...
            if hasattr(summary, "sections_after"):
                ...
                lines.append(
                    f"{pipe}     Tokens: {summary.after_token_count} ({delta:+d})"
                )
                lines.append(
                    f"{pipe}     Changed: {', '.join(summary.sections_changed[:3])}"
                )
            lines.append(f"{pipe}")
    ...
    def format_preview(
        ...
        lines.append(
            f"Current: {len(current_content)} chars, {len(current_lines)} lines"
        )
