...
def compile_prompt(
    ...
    if export_path:
        exp_compilers = (
            get_full_compilers() if include_dynamic else get_default_compilers()
        )
...
def _run_auto_improve(
    ...
    try:
        ...
        if suggestions:
            ...
            print(
                "(Auto-apply not implemented - use --feedback to apply specific changes)"
            )
        else:
...
def _generate_improvement_suggestions(
    ...
    if policy.verbosity < 0.4:
        if len(content) > 10000:
            suggestions.append(
                "Consider condensing content (your style prefers conciseness)"
            )
    elif policy.verbosity > 0.7:
        suggestions.append(
            "Consider adding more detail (your style prefers thoroughness)"
        )
    ...
    if policy.domain_focus:
        ...
        if focus > 0.6:
            suggestions.append(
                f"Consider emphasizing {domain_name} content (high focus area)"
            )
...
def compare_prompts(compiled_path: Path | None = None) -> None:
    ...
    print("\n" + "-" * 40)
    print(
        f"Hand-written length: {len(hand_written)} chars (~{len(hand_written) // 4} tokens)"
    )
...
def do_rollback(checkpoint_id: str) -> None:
    ...
    if len(checkpoint_id) < 8:
        print(
            f"Error: Checkpoint ID must be at least 8 characters (got {len(checkpoint_id)})"
        )
        sys.exit(1)
...
def main() -> None:
    ...
    compile_parser = subparsers.add_parser(
        "compile", help="Compile CLAUDE.md from sections"
    )
    compile_parser.add_argument(
        "--output", "-o", type=Path, help="Output path for compiled prompt"
    )
    compile_parser.add_argument(
    ...
    rollback_parser = subparsers.add_parser(
        "rollback", help="Rollback to a previous checkpoint"
    )
    ...
    diff_parser = subparsers.add_parser(
        "diff", help="Show diff between two checkpoints"
    )
