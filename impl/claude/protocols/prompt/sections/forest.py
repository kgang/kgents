...
class ForestSource(SectionSource):
    ...
    async def fetch(self, context: "CompilationContext") -> SourceResult:
        ...
        if context.focus_intent and active_trees:
            ...
            active_trees = self._filter_by_intent(
                active_trees, context.focus_intent, traces
            )
    ...
    def _extract_active_trees(
        self, content: str, traces: list[str]
    ) -> list[dict[str, str]]:
        ...
        row_pattern = r"\|\s*([^|]+)\s*\|\s*(\d+%)\s*\|\s*([^|]+)\s*\|\s*([^|]+)\s*\|\s*([^|]*)\s*\|"
    ...
    def _filter_by_intent(
        ...
        filtered = [
            t
            for t in trees
            if intent_lower in t["plan"].lower()
            or intent_lower in t.get("notes", "").lower()
        ]
