...
class ForestNode(BaseLogosNode):
    ...
    @aspect(
        ...
        help="View forest canopy (all plans)",
        long_help="Show summary of all plans in the forest, grouped by status.",
        examples=["kg forest", "kg forest --json"],
        see_also=["status", "witness"],
    )
    ...
    @aspect(
        category=AspectCategory.PERCEPTION,
        effects=[Effect.READS("plans")],
        help="View forest from YAML headers",
        long_help="Generate forest manifest by parsing YAML frontmatter from all plan files.",
        examples=["kg forest manifest"],
        see_also=["status", "witness"],
    )
    ...
    @aspect(
        category=AspectCategory.PERCEPTION,
        effects=[Effect.READS("plans")],
        help="Show implementation status matrix",
        long_help="Generate implementation status grouped by plan prefix with test counts.",
        examples=["kg forest status"],
        see_also=["manifest", "witness"],
    )
    ...
    @aspect(
        category=AspectCategory.PERCEPTION,
        effects=[Effect.READS("plans"), Effect.READS("tests")],
        help="Show drift between documented and actual state",
        long_help="Compare documented test counts and plan statuses against actual code state.",
        examples=["kg forest witness"],
        see_also=["manifest", "reconcile"],
    )
    ...
    @aspect(
        ...
        help="Archive stale plans (dry run by default)",
        long_help="Move dormant plans (>30 days untouched) to archive. Use --execute to apply.",
        examples=["kg forest tithe", "kg forest tithe --execute"],
        see_also=["sip", "reconcile"],
    )
    async def _tithe(
        self,
        observer: "Umwelt[Any, Any]",
    ...
    @aspect(
        ...
        effects=[Effect.WRITES("plans"), Effect.WRITES("git")],
        help="Regenerate _forest.md from plan headers",
        long_help="Full reconciliation: regenerate meta files and optionally commit changes.",
        examples=["kg forest reconcile", "kg forest reconcile --commit"],
        see_also=["manifest", "witness", "tithe"],
    )
...
"""
    ...
    @aspect(
        category=AspectCategory.ENTROPY,
        effects=[Effect.READS("plans")],
        help="Select dormant plan from accursed share",
        long_help="Randomly select a dormant plan for attention (entropy budget allocation).",
        examples=["kg forest sip"],
        see_also=["tithe", "dream"],
    )
    async def _sip(
        self,
        observer: "Umwelt[Any, Any]",
    ...
    @aspect(
        ...
        effects=[Effect.READS("plans")],
        help="Propose plan mutation with rollback",
        long_help="Generate a preview of proposed changes with a rollback token for apply/cancel.",
        examples=["kg forest refine agents/k-gent --progress 95"],
        see_also=["apply", "rollback"],
    )
    async def _refine(
        self,
        observer: "Umwelt[Any, Any]",
    ...
    @aspect(
        category=AspectCategory.GENERATION,
        effects=[Effect.WRITES("plans")],
        help="Create a new plan scaffold",
        long_help="Generate a new plan file with proper YAML frontmatter structure.",
        examples=["kg forest define agents/new-gent --title 'New Agent'"],
        see_also=["manifest", "refine"],
    )
    async def _define(
        self,
        observer: "Umwelt[Any, Any]",
