...
def test_extract_imports_complex():
...
def test_branching_factor_simple():
    tree = ast.parse(SIMPLE_CODE)
    bf = estimate_branching_factor(tree)
    assert bf == 1  # No branches


def test_branching_factor_complex():
    tree = ast.parse(COMPLEX_CODE)
    bf = estimate_branching_factor(tree)
    assert bf >= 1
...
def test_recursion_with_base_case_allowed():
...
def test_loop_with_break_allowed():
