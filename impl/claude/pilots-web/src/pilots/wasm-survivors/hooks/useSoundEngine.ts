...
export interface UseSoundEngineResult {
  ...
  /** Play kill sound with pitch variation based on enemy type */
  playKill: (enemyType: 'basic' | 'fast' | 'tank' | 'boss') => void;
  /** Play damage sound */
  playDamage: () => void;
...
export function useSoundEngine(): UseSoundEngineResult {
  ...
  const playKill = useCallback((enemyType: 'basic' | 'fast' | 'tank' | 'boss') => {
    // Vary pitch based on enemy type for variety
