...
The fundamental reactive primitives that enable target-agnostic widgets.
...
class Signal(Generic[T]):
    ...
    def map(self, fn: Callable[[T], U]) -> Computed[U]:
        ...
        This is the Functor instance for Signal:
        map: (T -> U) -> Signal[T] -> Computed[U]
...
class Computed(Generic[T]):
    ...
    def value(self) -> T:
        ...
        if self._dirty:
            ...
            self._dirty = False
        # Cast is safe because we just computed
    ...
    def map(self, fn: Callable[[T], U]) -> Computed[U]:
        ...
        Note: Creates a new Computed that depends on this one's computation,
        not on this Computed itself (since Computed isn't a Signal).
        For full reactive chaining, use the original Signal sources.
        """
