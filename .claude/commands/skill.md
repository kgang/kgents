...
### If argument provided (e.g., `/skill polynomial-agent`)
...
```
/skill polynomial-agent    → Read the polynomial agent skill
/skill agentese-node      → Read AGENTESE node registration
/skill                    → Show routing table, ask what task
/skill elastic-ui         → Read elastic UI patterns
```
