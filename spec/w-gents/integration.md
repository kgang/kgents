...
### Use Case: Observing Evolution Session
...
1. E-gent starts evolution loop
2. W-gent automatically attaches (due to `--observe` flag)
3. Browser opens showing evolution stages

**W-gent Dashboard**:

```
┌─ evolution :: preflight.py ───────────────────────┐
│                                                   │
│ ┌─ Pipeline Stages ──────────────────────────┐   │
...
│                                                   │
│ ┌─ Current Experiment ───────────────────────┐   │
│ │ Hypothesis: "Add type hints to validate()" │   │
│ │ Status: Running mypy...                    │   │
│ │ Attempt: 1/5                               │   │
│ └───────────────────────────────────────────┘   │
│                                                   │
...
│ │ Recently rejected:                         │   │
│ │ - "Refactor to use dataclass" (too risky)  │   │
│ │ - "Add logging" (not tasteful)             │   │
│ └───────────────────────────────────────────┘   │
└────────────────────────────────────────────────────┘
```

### E-gent Wire Protocol

E-gent exposes structured state:
...
```json
{
  ...
  "max_attempts": 5,
  "recent_rejections": [
    {"hypothesis": "Refactor to use dataclass", "reason": "too risky"},
    {"hypothesis": "Add logging", "reason": "not tasteful"}
  ]
}
```
...
- [../e-gents/](../e-gents/) - Evolution agents (uses W-gent for observation)
