...
## E-gent Integration: Learning from Lineage

L-gent + E-gent synergy: Use lineage to improve future evolutions.

```python
class EvolutionLearner:
    """Learn from lineage to improve E-gent hypothesis generation."""
    ...
    async def get_successful_patterns(
        self,
        artifact_type: EntityType,
        evolution_type: str | None = None
    ) -> list[EvolutionPattern]:
        """
        Find evolution patterns that worked in the past.

        Returns patterns that led to successful improvements.
        """
        # Query all successor_to edges
        all_evolutions = await self.lineage.storage.query_edges(
            relationship_type="successor_to"
        )

        # Filter to successful evolutions
        successful = [
            e for e in all_evolutions
            if e.context.get("success", True)
        ]

        # Group by evolution_type
        patterns = defaultdict(list)
        for edge in successful:
            etype = edge.context.get("evolution_type", "unknown")
            if evolution_type is None or etype == evolution_type:
                patterns[etype].append(edge)

        # Build pattern objects with statistics
        result = []
        for etype, edges in patterns.items():
            avg_improvement = self._avg_improvement(edges)
            result.append(EvolutionPattern(
                evolution_type=etype,
                count=len(edges),
                average_improvement=avg_improvement,
                examples=[e.source_id for e in edges[:5]]
            ))

        return sorted(result, key=lambda p: p.average_improvement, reverse=True)
    ...
    async def suggest_evolution(
        ...
        """
        Suggest evolution strategies based on what worked for similar artifacts.
        """
        # Find similar artifacts
        similar = await self.l_gent.find_similar(artifact_id, limit=10)

        # Check their evolution history
        suggestions = []
        for sim in similar:
            history = await self.lineage.get_evolution_history(sim.id)
            for step in history:
                if step.success:
                    suggestions.append(EvolutionSuggestion(
                        evolution_type=step.evolution_type,
                        precedent_id=sim.id,
                        improvement=step.metrics.get("improvement", 0),
                        reason=f"Similar artifact {sim.id} improved via {step.evolution_type}"
                    ))
...
class EvolutionPattern:
    """A pattern of successful evolution."""
    evolution_type: str
    count: int                    # How many times this worked
    average_improvement: float    # Average metric improvement
    examples: list[str]           # Example artifact IDs

@dataclass
class EvolutionSuggestion:
    """A suggested evolution based on precedent."""
    evolution_type: str
    precedent_id: str             # Which artifact demonstrated this
...
- [../e-gents/](../e-gents/) - Evolution that creates lineage
