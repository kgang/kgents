...
### Ephemeral Agents
- Created for a single task instance
- Not persisted to spec/ or impl/
- Compiled just-in-time, garbage collected after use
- May be cached by hash for reuse within session
...
- **Cached**: May be reused if same (intent, context) encountered
...
2. Validates it (Chaosmonger → Judge)
